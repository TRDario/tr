...
template <tr::enumerator T> auto tr::enum_hash<T>::operator()(T arg) const noexcept
{
	return std::hash<std::underlying_type_t<T>>{}(std::underlying_type_t<T>(arg));
}

auto tr::string_hash::operator()(std::string_view str) const noexcept
{
	return std::hash<std::string_view>{}(str);
}

constexpr bool tr::string_eq::operator()(std::string_view l, std::string_view r) const noexcept
{
	return l == r;
}
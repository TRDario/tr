/// @file
/// @brief Implements the templated parts of logger.hpp.
...
void tr::logger::log(severity severity, std::format_string<Args...> fmt, Args&&... args)
{
	log(severity, std::format(fmt, std::forward<Args>(args)...));
}

template <typename... Args>
void tr::logger::log_continue(std::format_string<Args...> fmt, Args&&... args)
{

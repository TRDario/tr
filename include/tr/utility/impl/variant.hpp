/// @file
/// @brief Implements variant.hpp.

#pragma once
#include "../variant.hpp"

//

template <typename Alternative, typename... Alternatives>
	requires(tr::one_of<Alternative, Alternatives...>)
constexpr tr::opt_ref<Alternative> tr::get_if(std::variant<Alternatives...>& v) noexcept
{
	return make_opt_ref(std::get_if<Alternative>(&v));
}

template <typename Alternative, typename... Alternatives>
	requires(tr::one_of<Alternative, Alternatives...>)
constexpr std::optional<Alternative> tr::get_if(std::variant<Alternatives...>&& v) noexcept
{
	Alternative* const ptr{std::get_if<Alternative>(&v)};
	return ptr != nullptr ? std::make_optional<Alternative>(std::move(*ptr)) : std::nullopt;
}

template <typename Alternative, typename... Alternatives>
	requires(tr::one_of<Alternative, Alternatives...>)
constexpr tr::opt_ref<const Alternative> tr::get_if(const std::variant<Alternatives...>& v) noexcept
{
	return make_opt_ref(std::get_if<Alternative>(&v));
}

template <typename Alternative, typename... Alternatives>
	requires(tr::one_of<Alternative, Alternatives...>)
constexpr std::optional<const Alternative> tr::get_if(const std::variant<Alternatives...>&& v) noexcept
{
	const Alternative* const ptr{std::get_if<Alternative>(&v)};
	return ptr != nullptr ? std::make_optional<const Alternative>(std::move(*ptr)) : std::nullopt;
}

//

template <typename Alternative, typename... Alternatives, std::invocable<Alternative&> Fn>
constexpr void tr::if_is(std::variant<Alternatives...>& v, Fn&& fn) noexcept(noexcept(fn(std::declval<Alternative&>())))
{
	tr::opt_ref<Alternative> ref{get_if<Alternative>(v)};
	if (ref.has_ref()) {
		(void)fn(*ref);
	}
}

template <typename Alternative, typename... Alternatives, std::invocable<Alternative> Fn>
constexpr void tr::if_is(std::variant<Alternatives...>&& v, Fn&& fn) noexcept(noexcept(fn(std::declval<const Alternative&>())))
{
	std::optional<Alternative> value{get_if<Alternative>(std::move(v))};
	if (value.has_value()) {
		(void)fn(std::move(*value));
	}
}

template <typename Alternative, typename... Alternatives, std::invocable<const Alternative&> Fn>
constexpr void tr::if_is(const std::variant<Alternatives...>& v, Fn&& fn) noexcept(noexcept(fn(std::declval<Alternative>())))
{
	tr::opt_ref<const Alternative> ref{get_if<Alternative>(v)};
	if (ref.has_ref()) {
		(void)fn(*ref);
	}
}

template <typename Alternative, typename... Alternatives, std::invocable<const Alternative> Fn>
constexpr void tr::if_is(const std::variant<Alternatives...>&& v, Fn&& fn) noexcept(noexcept(fn(std::declval<const Alternative>())))
{
	std::optional<const Alternative> value{get_if<Alternative>(std::move(v))};
	if (value.has_value()) {
		(void)fn(std::move(*value));
	}
}
/// @file
/// @brief Implements the templated parts of rng.hpp.
...
Integer tr::rng::generate() noexcept
{
...
template <std::integral Integer>
Integer tr::rng::generate(Integer max) noexcept
{
...
template <std::integral Integer>
Integer tr::rng::generate(Integer min, Integer max) noexcept
{
...
FloatingPoint tr::rng::generate() noexcept
{
	return static_cast<FloatingPoint>(advance()) / static_cast<FloatingPoint>(UINT64_MAX);
}

template <std::floating_point FloatingPoint>
FloatingPoint tr::rng::generate(FloatingPoint max) noexcept
{
	return generate<FloatingPoint>() * max;
}

template <std::floating_point FloatingPoint>
FloatingPoint tr::rng::generate(FloatingPoint min, FloatingPoint max) noexcept
{
	TR_ASSERT(min < max, "RNG range minimum must be less than the maximum (Currenly: {} !< {})", min, max);
...
template <std::same_as<tr::angle>>
tr::angle tr::rng::generate() noexcept
{
	return turns(generate<float>());
}

template <std::same_as<glm::vec2>>
glm::vec2 tr::rng::generate(float magnitude) noexcept
{
...
template <std::same_as<glm::vec2>>
glm::vec2 tr::rng::generate(const tr::rectangle<float>& region) noexcept
{

...
template <std::integral Integer>
Integer tr::rng::generate(Integer max)
{
...
template <std::integral Integer>
Integer tr::rng::generate(Integer min, Integer max)
{
...
FloatingPoint tr::rng::generate()
{
	return static_cast<FloatingPoint>(advance()) / static_cast<FloatingPoint>(UINT64_MAX);
}

template <std::floating_point FloatingPoint>
FloatingPoint tr::rng::generate(FloatingPoint max)
{
	return generate<FloatingPoint>() * max;
}

template <std::floating_point FloatingPoint>
FloatingPoint tr::rng::generate(FloatingPoint min, FloatingPoint max)
{
	TR_ASSERT(min < max, "RNG range minimum must be less than the maximum (Currenly: {} !< {})", min, max);
...
tr::angle tr::rng::generate()
{
	return turns(generate<float>());
}

template <std::same_as<glm::vec2>>
glm::vec2 tr::rng::generate(float magnitude)
{
...
template <std::same_as<glm::vec2>>
glm::vec2 tr::rng::generate(const tr::rectangle<float>& region)
{

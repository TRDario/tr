...
namespace tr
{
	...
	class window
	{
	  public:
		...
		[[nodiscard]] operator window_view() noexcept;

		/// Creates a view to the window.
		/// @return View to the window.
		[[nodiscard]] window_view view() noexcept;
	  ...
	  private:
		...
		struct deleter
		{
			...
			static void operator()(SDL_Window* ptr) noexcept;
		};
		...
		/// @return View to the window.
		[[nodiscard]] window_view view() const noexcept;
	};
} // namespace tr
	...
	, m_range{range}
{
	...
	const usize vertices{std::ranges::fold_left(range, 0uz, [](usize s, const mesh& m) { return s + m.positions.size(); })};
	const usize indices{std::ranges::fold_left(range, 0uz, [](usize s, const mesh& m) { return s + m.indices.size(); })};

...
namespace tr
{
	namespace
	{
		/// Unique ID of the next shader object.
		unsigned int next_shader_tid{0};
	} // namespace
} // namespace tr
#endif
...
{
#ifdef TR_ENABLE_GL_CHECKS
	context.registry.register_shader((m_program.get_deleter().tid = next_shader_tid++), gid());
#endif

	const gl_api& gl{context.gl()};

	int linked;
	gl.get_program_iv(gid(), GL_LINK_STATUS, &linked);
	if (!linked) {
		int info_log_buffer_size;
		gl.get_program_iv(gid(), GL_INFO_LOG_LENGTH, &info_log_buffer_size);
		std::string info_log_buffer(info_log_buffer_size, '\0');
		gl.get_program_info_log(gid(), info_log_buffer_size, nullptr, info_log_buffer.data());
...
{
	int uniforms{0};
	gl.get_program_interface_iv(gid(), GL_UNIFORM, GL_ACTIVE_RESOURCES, &uniforms);
	for (int i = 0; i < uniforms; ++i) {
		std::array<int, uniform_properties.size()> values;
		const auto& [block_index, var_type, array_size, name_length, location]{values};
		gl.get_program_resource_iv(gid(), GL_UNIFORM, i, uniform_properties.size(), uniform_properties.data(), uniform_properties.size(),
								   NULL, values.data());
		...
		gl.get_program_resource_name(gid(), GL_UNIFORM, i, uniform_name_buffer.size(), NULL, uniform_name_buffer.data());
		m_uniforms.insert({
...
{
	int inputs{0};
	gl.get_program_interface_iv(gid(), GL_PROGRAM_INPUT, GL_ACTIVE_RESOURCES, &inputs);
	for (int i = 0; i < inputs; ++i) {
		std::array<int, input_output_properties.size()> values;
		const auto& [var_type, array_size, name_length, location]{values};
		gl.get_program_resource_iv(gid(), GL_PROGRAM_INPUT, i, input_output_properties.size(), input_output_properties.data(),
								   input_output_properties.size(), NULL, values.data());

		std::string input_name_buffer(name_length, '\0');
		gl.get_program_resource_name(gid(), GL_PROGRAM_INPUT, i, input_name_buffer.size(), NULL, input_name_buffer.data());
		m_inputs.insert({
			static_cast<unsigned int>(location),
...
{
	int inputs{0};
	gl.get_program_interface_iv(gid(), GL_PROGRAM_OUTPUT, GL_ACTIVE_RESOURCES, &inputs);
	for (int i = 0; i < inputs; ++i) {
		std::array<int, input_output_properties.size()> values;
		const auto& [var_type, array_size, name_length, location]{values};
		gl.get_program_resource_iv(gid(), GL_PROGRAM_OUTPUT, i, input_output_properties.size(), input_output_properties.data(),
								   input_output_properties.size(), NULL, values.data());

		std::string output_name_buffer(name_length, '\0');
		gl.get_program_resource_name(gid(), GL_PROGRAM_OUTPUT, i, output_name_buffer.size(), NULL, output_name_buffer.data());
...
void tr::shader::set_uniform(int index, bool value)
{
	...
	context().gl().set_program_uniform_1i(gid(), index, value);
}
...
void tr::shader::set_uniform(int index, std::span<const int> value)
{
	...
	context().gl().set_program_uniform_1iv(gid(), index, value.size(), value.data());
}
...
void tr::shader::set_uniform(int index, glm::ivec2 value)
{
	...
	context().gl().set_program_uniform_2i(gid(), index, value.x, value.y);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::ivec2> value)
{
	...
	context().gl().set_program_uniform_2iv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, glm::ivec3 value)
{
	...
	context().gl().set_program_uniform_3i(gid(), index, value.x, value.y, value.z);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::ivec3> value)
{
	...
	context().gl().set_program_uniform_3iv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, glm::ivec4 value)
{
	...
	context().gl().set_program_uniform_4i(gid(), index, value.x, value.y, value.z, value.w);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::ivec4> value)
{
	...
	context().gl().set_program_uniform_4iv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, unsigned int value)
{
	...
	context().gl().set_program_uniform_1ui(gid(), index, value);
}
...
void tr::shader::set_uniform(int index, std::span<const unsigned int> value)
{
	...
	context().gl().set_program_uniform_1uiv(gid(), index, value.size(), value.data());
}
...
void tr::shader::set_uniform(int index, glm::uvec2 value)
{
	...
	context().gl().set_program_uniform_2ui(gid(), index, value.x, value.y);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::uvec2> value)
{
	...
	context().gl().set_program_uniform_2uiv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, glm::uvec3 value)
{
	...
	context().gl().set_program_uniform_3ui(gid(), index, value.x, value.y, value.z);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::uvec3> value)
{
	...
	context().gl().set_program_uniform_3uiv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, glm::uvec4 value)
{
	...
	context().gl().set_program_uniform_4ui(gid(), index, value.x, value.y, value.z, value.w);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::uvec4> value)
{
	...
	context().gl().set_program_uniform_4uiv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, float value)
{
	...
	context().gl().set_program_uniform_1f(gid(), index, value);
}
...
void tr::shader::set_uniform(int index, std::span<const float> value)
{
	...
	context().gl().set_program_uniform_1fv(gid(), index, value.size(), value.data());
}
...
void tr::shader::set_uniform(int index, glm::vec2 value)
{
	...
	context().gl().set_program_uniform_2f(gid(), index, value.x, value.y);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::vec2> value)
{
	...
	context().gl().set_program_uniform_2fv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, glm::vec3 value)
{
	...
	context().gl().set_program_uniform_3f(gid(), index, value.x, value.y, value.z);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::vec3> value)
{
	...
	context().gl().set_program_uniform_3fv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, glm::vec4 value)
{
	...
	context().gl().set_program_uniform_4f(gid(), index, value.x, value.y, value.z, value.w);
}
...
void tr::shader::set_uniform(int index, std::span<const glm::vec4> value)
{
	...
	context().gl().set_program_uniform_4fv(gid(), index, value.size(), value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat2& value)
{
	...
	context().gl().set_program_uniform_matrix2fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat2> value)
{
	...
	context().gl().set_program_uniform_matrix2fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat3& value)
{
	...
	context().gl().set_program_uniform_matrix3fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat3> value)
{
	...
	context().gl().set_program_uniform_matrix3fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat4& value)
{
	...
	context().gl().set_program_uniform_matrix4fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat4> value)
{
	...
	context().gl().set_program_uniform_matrix4fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat2x3& value)
{
	...
	context().gl().set_program_uniform_matrix2x3fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat2x3> value)
{
	...
	context().gl().set_program_uniform_matrix2x3fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat2x4& value)
{
	...
	context().gl().set_program_uniform_matrix2x4fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat2x4> value)
{
	...
	context().gl().set_program_uniform_matrix2x4fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat3x2& value)
{
	...
	context().gl().set_program_uniform_matrix3x2fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat3x2> value)
{
	...
	context().gl().set_program_uniform_matrix3x2fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat3x4& value)
{
	...
	context().gl().set_program_uniform_matrix3x4fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat3x4> value)
{
	...
	context().gl().set_program_uniform_matrix3x4fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat4x2& value)
{
	...
	context().gl().set_program_uniform_matrix4x2fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat4x2> value)
{
	...
	context().gl().set_program_uniform_matrix4x2fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, const glm::mat4x3& value)
{
	...
	context().gl().set_program_uniform_matrix4x3fv(gid(), index, 1, false, value_ptr(value));
}
...
void tr::shader::set_uniform(int index, std::span<const glm::mat4x3> value)
{
	...
	context().gl().set_program_uniform_matrix4x3fv(gid(), index, value.size(), false, value_ptr(value[0]));
}
...
void tr::shader::set_uniform(int index, texture_view texture)
{
	...
	if (unit_it == m_texture_units.end()) {
		...
		context().gl().set_program_uniform_1i(gid(), index, unit_it->second.id());
	}
...
void tr::shader::set_label(std::string_view label)
{
	...
	context().gl().set_object_label(GL_PROGRAM, gid(), label.size(), label.data());
}
...
std::string tr::shader::label() const
{
	...
	int label_length;
	gl.get_object_label(GL_PROGRAM, gid(), 0, &label_length, nullptr);
	if (label_length > 0) {
		std::string label_string(label_length, '\0');
		gl.get_object_label(GL_PROGRAM, gid(), label_length + 1, nullptr, label_string.data());
		return label_string;
	}
	else {
		return "<unnamed>";
...
unsigned int tr::shader::gid() const
{
	return m_program.get();
}

#ifdef TR_ENABLE_GL_CHECKS
unsigned int tr::shader::tid() const
{
	return m_program.get_deleter().tid;
}

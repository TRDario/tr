...
tr::system::tick_event::tick_event(u32 id)
	: id{id}
{
}
...
tr::system::custom_event_base::custom_event_base(u32 type)
	: type{type}, uint{}, sint{}
{
}

tr::system::custom_event_base::custom_event_base(u32 type, u32 uint, i32 sint)
	: type{type}, uint{uint}, sint{sint}
{
}

tr::system::custom_event_base::custom_event_base(u32 type, u32 uint, i32 sint, std::any&& any1, std::any&& any2)
...
tr::system::custom_event_base::custom_event_base(const event& event)
{
	...
	const SDL_UserEvent& sdl{((const SDL_Event*)&event)->user};
	type = sdl.type;
	uint = sdl.windowID;
	sint = sdl.code;
...
tr::system::custom_event_base::custom_event_base(event&& event)
{
	...
	SDL_UserEvent& sdl{((SDL_Event*)&event)->user};
	type = sdl.type;
	uint = sdl.windowID;
	sint = sdl.code;
	if (sdl.data1 != nullptr) {
...
tr::system::event::event(const event& r)
{
	...
	if (type() >= SDL_EVENT_USER) {
		...
		const SDL_UserEvent& rsdl{((const SDL_Event*)r.m_buffer)->user};
		lsdl.data1 = lsdl.data1 != nullptr ? new std::any{*(const std::any*)rsdl.data1} : nullptr;
		lsdl.data2 = lsdl.data2 != nullptr ? new std::any{*(const std::any*)rsdl.data2} : nullptr;
	}
}
...
{
	if (type() >= SDL_EVENT_USER) {
		SDL_UserEvent& sdl{((SDL_Event*)m_buffer)->user};
		delete (std::any*)sdl.data1;
		delete (std::any*)sdl.data2;
	}
}
...
tr::timer tr::system::create_tick_timer(float frequency, u32 id)
{
...
std::optional<tr::system::event> tr::system::poll_event()
{
	...
	event event;
	return SDL_PollEvent((SDL_Event*)&event) ? std::optional<system::event>{std::move(event)} : std::nullopt;
}
...
std::optional<tr::system::event> tr::system::wait_for_event(imsecs timeout)
{
	...
	event event;
	if (SDL_WaitEventTimeout((SDL_Event*)&event, i32(timeout.count()))) {
		return event;
	}
	else {
		return std::nullopt;
...
void tr::system::push_event(const event& event)
{
	...
	SDL_Event sdl{*(const SDL_Event*)&event};
	if (event.type() >= SDL_EVENT_USER) {
		const SDL_UserEvent& rsdl{((const SDL_Event*)&event)->user};
...
void tr::system::push_event(event&& event)
{
	...
	if (event.type() >= SDL_EVENT_USER) {
		SDL_UserEvent& rsdl{((SDL_Event*)&event)->user};
		rsdl.data1 = nullptr;
		rsdl.data2 = nullptr;
	}
}